...
export class DetectBlackService {
  ...
  recursiveBlackDetect(mediaFile: File, position: string) {
    ...
    if (position === 'head' && this.headBlackTryCount >= this.MAX_TRIES) {
      console.log(`%c Max retries exceeded for head black detection in file: ${position}`, 'color: orange');
    ...
    if (position === 'tail' && this.tailBlackTryCount >= this.MAX_TRIES) {
      console.log(`%c Max retries exceeded for tail black detection in file: ${position}`, 'color: orange');
      // TODO add alert to DOM
    ...
    if (position === 'head') {
      ...
      console.log(
        `%c Head try count: ${this.headBlackTryCount}, \t slice start: ${sliceStart}, \t slice end: ${sliceEnd}`,
        'color: grey',
      );
    }
    if (position === 'tail') {
      ...
      console.log(
        `%c Tail try count: ${this.tailBlackTryCount}, \t tail slice start: ${tailSliceStart}, \t tail slice end: ${tailSliceEnd}`,
        'color: grey',
      );
    }
    ...
    if (position === 'head') {
      ...
      console.log(
        `%c Slice for head black detect:\t type: ${sliceToUse.type}, \t size: ${sliceToUse.size}`,
        'color: grey',
      );
    ...
    $.when(this.requestBlack(sliceToUse, position, fileToUse)).then((data) => {
      if (data.blackDetect[0]) {
        ...
        console.log(`%c This is my black duration, returned from requestBlack:: ${duration}`, 'color: grey');
        ...
        if (position === 'head') {
          ...
          if (this.headBlackPrevDuration && this.headBlackPrevDuration <= duration) {
            ...
            console.log(` $cBlack detection: duration isn't getting longer. TODO - stop here`, 'color: lime');
          }
        }
        if (position === 'tail') {
        ...
        if (duration >= this.MIN_BLACK_TIME) {
          console.log(
            `%c The detected black duration of: ${duration} is greater or equal to the min black time of: ${this.MIN_BLACK_TIME}`,
            'color: grey',
          );

          console.log(`%c So we can stop recursing.`, 'color: darkgrey');
        ...
        console.log(`%c No blackdetect object on the returned array`, 'color: grey');
      }
      ...
      if (position === 'tail') {
        console.log(`%c TailBlackTryCount: ${this.tailBlackTryCount}`, 'color: grey');
  ...
  requestBlack(slice: any, position: string, filename: string): any {
    return $.ajax({
      ...
      error: (err) => {
        console.log(`%c Error on the black detection ajax request:`, 'color: red');
        console.log(err);
      },
      success: (data) => {
        console.log(`%c From requestBlack, for the: ${position}`, 'color: green');

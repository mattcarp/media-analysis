...
export class ValidationService {
  ...
  imageRules = validationRules.imageStream;
  ...
  validate(fileId: string, data: { analysis; error }, err, fileType: string): void {
    if (data.analysis) {
      ...
      if (audioStream && fileType === 'video' || fileType === 'audio') {
      ...
      if (videoStream && fileType === 'image') {
        ...
        this.imageValidations.push({
          name: 'Width',
          value: videoStream.width,
          success: videoStream.width >= this.imageRules.width[0] &&
            this.imageRules.width[1] >= videoStream.width,
          message: this.imageRules.widthMessage
        });

        this.imageValidations.push({
          name: 'Height',
          value: videoStream.height,
          success: videoStream.height >= this.imageRules.height[0] &&
            this.imageRules.height[1] >= videoStream.height,
          message: this.imageRules.heightMessage
        });
      }
      ...
      if (fileType === 'video') {
        validations = this.videoValidations.concat(this.audioValidations);
      }
      if (fileType === 'audio') { validations = this.audioValidations }
      if (fileType === 'image') { validations = this.imageValidations }

...
export class CdTextService {
    ...
    for (let i = 0; i < parsedPacks.length; i++) {
      ...
      if (trackChanged) {
        ...
        if (parsedPacks[i - 1].charPos !== 15) {
          ...
          this.logger.log(
            'my arithmetic:',
            PAYLOAD_LENGTH - parsedPacks[i].charPos,
          );
  ...
  assemblePacks(parsedPacks: any[]) {
    ...
    for (let i = 0; i < parsedPacks.length; i++) {
      ...
      i < parsedPacks.length - 1
        ? (nextCharPos = parsedPacks[i + 1].charPos)
        : (nextCharPos = -1);
      // this.logger.log('prior, current, and next charPos', priorCharPos, currCharPos, nextCharPos);
      if (
        i > 0 &&
        parsedPacks[i].packTrackNo !== parsedPacks[i - 1].packTrackNo
      ) {
        trackChanged = true;
      ...
      if (currCharPos === 15) {
        this.logger
          .log(`need to get the entire previous pack, and if prev pack's charpos is > 0,
          will need to go back more`);
      }
    }
    this.store.dispatch(
      setParsedPackItems({ parsedPackItems: this.assembledEntries }),
    );
    this.logger.log(
      'called next on assembled with this',
      this.assembledEntries,
    );
  }
  ...
  writePriorPayload(currentPos: number, parsedPacks: Array<any>) {
    ...
    if (prevPack.charPos === 0 && nextPack) {
      this.logger.log(
        'charPos on previous pack is 0, so we should write the payload',
      );
      this.logger.log('we are about to crash. nextPack is', nextPack);
      assembledPayload = currPack.payload.slice(
        0,
        PAYLOAD_LENGTH - nextPack.charPos,
      );
    }
    if (prevPack.charPos > 0 && prevPack.charPos < 15) {
      this.logger.log(
        'should i be slicing this payload to get to *give me a reason?',
        parsedPacks[currentPos - 2].payload,
      );
      assembledPayload = parsedPacks[currentPos - 2].payload.slice(
        PAYLOAD_LENGTH - prevPack.charPos,
      );
      assembledPayload += prevPack.payload.slice(
        0,
    ...
    if (prevPack.charPos === 15) {
      if (parsedPacks[currentPos - 2].charPos > 0) {
        ...
        this.logger.log(
          `handling a case where there's a 15 charPos preceded by a 12:`,
          assembledPayload,
        );
      }
    }
  ...
  parsePack(pack: ArrayBuffer) {
    ...
    parsedPack.payload = String.fromCharCode.apply(
      null,
      new Uint8Array(payloadBin),
    );
  ...
  getFile(cdTextFileInfo: any, allFiles: Array<any>) {
    ...
    for (const fileObj of allFiles) {
      if (fileObj.fileName.toLowerCase() === fileName) {
        this.logger.log(
          'i think i have the cd text resumable file obj:',
          fileObj,
        );
  ...
  readAsBinary(cdTextFile: File): void {

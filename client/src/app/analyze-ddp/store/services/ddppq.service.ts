...
export class DdppqService {
  ...
  static parseSingleBlock(block): any {
    const blockObj: any = {};
  ...
  insertGaps(pqArr): any {
    ...
    for (let i = 0; i < pqArr.length; i++) {
      ...
      if (pqArr[i].idx.trim() === '01' && pqArr[i].trk.trim() !== 'AA') {
        const pauseStart =
          pqArr[i - 1].min + ':' + pqArr[i - 1].sec + ':' + pqArr[i - 1].frm;
        const trackStart =
          pqArr[i].min + ':' + pqArr[i].sec + ':' + pqArr[i].frm;
        const startFrame = this.timeToFrames(pauseStart);
        const endFrame = this.timeToFrames(trackStart);
        pqArrWithGaps[i].preGap = endFrame - startFrame;
      }
    }
  ...
  insertDurations(pqArr: PqEntry[]): any {
    ...
    for (let i = 0; i < pqArr.length; i++) {
      ...
      if (pqArr[i].idx.trim() === '01' && pqArr[i].trk.trim() !== 'AA') {
        ...
        for (j; j < pqArr.length; j++) {
          ...
          const isLeadout =
            pqArr[j].trk.trim().toUpperCase() === 'AA' &&
            pqArr[j].idx.trim() === '01';
          if (
            (pqArr[j].idx.trim() === '00' &&
              currentTrack + 1 === parseInt(pqArr[j].trk, 10)) ||
            isLeadout
          ) {
            nextPauseStart =
              pqArr[j].min + ':' + pqArr[j].sec + ':' + pqArr[j].frm;
            break;
          }
        }
  ...
  addPqToAudio(audioEntries: any[], parsedPq: PqState): any {

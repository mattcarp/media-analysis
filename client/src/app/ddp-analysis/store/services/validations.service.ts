...
export class ValidationsService {
  ...
  validate(collectedFileInfo, parsedMs, parsedId) {
    ...
    const fileStatus: any = this.getFileStatus(collectedFileInfo, parsedMs, parsedId);
    ...
    validation.totalFails = fileStatus.missingFiles.length + sizeMismatches.length +
  ...
  getFileStatus(collectedFileInfo: any, parsedMs: MsState, parsedId: IdState) {
    ...
    for (let i = 0; i < requiredFiles.length; i++) {
      ...
      for (let j = 0; j < collectedFileInfo.length; j++) {
        ...
        if (reqFileName.trim().toLowerCase() === foundFileName.trim().toLowerCase()) {
          fileExists = true;
          ...
          if (reqFileSize === foundFileSize) {
            ...
            const sizeDiff: number = parseInt(foundFileSize, 10) - parseInt(reqFileSize, 10);
            sizeMismatches.push({
              name: reqFileName,
              reqSize: reqFileSize,
  ...
  getPqMsIssues(msArray, pqWithGapsArr) {
    const issues: any = [];
    let issue: any = {};

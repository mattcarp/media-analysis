...
@Component({
  ...
  templateUrl: './validate-format.component.html',
  styleUrls: ['./validate-format.component.scss'],
})
...
export class ValidateFormatComponent {
  ...
  validate(metadata: any) {
    ...
    const streams = analysisObj.streams;
  ...
  setIcon(item: any) {
    return {
      'fa-check-circle': item.pass,
      'fa-times-circle': !item.pass,
      'fa-exclamation-triangle': item.warn,
    };
  }
}
